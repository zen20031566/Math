...
public class SigmaGrassGenerator : MonoBehaviour
{
    ...
    private ComputeBuffer voteBuffer, scanBuffer, groupSumArrayBuffer, scannedGroupSumBuffer, argsBuffer;
    ...
    public class GrassChunk
    {
        ...
        public bool IsGrassDataBufferValid => grassDataBuffer != null && grassDataBuffer.IsValid();
    }
    ...
    void OnEnable()
    {
        ...
        int numPointsPerChunk = chunkSize * chunkSize;
        
        voteBuffer = new ComputeBuffer(numPointsPerChunk, sizeof(uint));
        scanBuffer = new ComputeBuffer(numPointsPerChunk, sizeof(uint));
        groupSumArrayBuffer = new ComputeBuffer(numPointsPerChunk, sizeof(uint));
        scannedGroupSumBuffer = new ComputeBuffer(numPointsPerChunk, sizeof(uint));
        //argsBuffer = new ComputeBuffer(1, sizeof(uint) * 5, ComputeBufferType.IndirectArguments);
        
        numThreadGroups = Mathf.CeilToInt(numPointsPerChunk / 128.0f);
        ...
        numVoteThreadGroups = Mathf.CeilToInt(numPointsPerChunk / 128.0f);
    ...
    void InitChunks()
    {
        chunks = new GrassChunk[numChunkPerEdge * numChunkPerEdge];

        for (int x = 0; x < numChunkPerEdge; ++x) 
        {
            for (int y = 0; y < numChunkPerEdge; ++y) 
            {
                chunks[x + y * numChunkPerEdge] = InitGrassChunk(x, y);
            }
        }
    }

    GrassChunk InitGrassChunk(int x, int y)
    {
