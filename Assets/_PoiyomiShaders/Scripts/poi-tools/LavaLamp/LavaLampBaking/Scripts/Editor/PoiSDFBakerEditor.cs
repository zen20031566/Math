...
[CustomEditor(typeof(PoiSDFBaker))]
public class PoiSDFBakerEditor : Editor
{
    private static bool sRenderersFoldout = true;
    private static bool[] sSubmeshFoldouts = new bool[100];

    private static PoiSDFBaker.DebugVisualization sVisualizationMode = PoiSDFBaker.DebugVisualization.Current;
    ...
    {
        PoiSDFBaker baker = (PoiSDFBaker)target;
    ...
    private void SettingsPanel()
    {
        ...
        List<PoiSDFBaker.TargetRendererInfo> newTargetRendererInfos;
        ...
        if (targetObject != null)
        {
            ...
            if (sRenderersFoldout)
            {
                ...
                for (int rendererIndex = 0; rendererIndex < newTargetRendererInfos.Count; rendererIndex++)
                {
                    ...
                    PoiSDFBaker.TargetRendererInfo currentTarget = newTargetRendererInfos[rendererIndex];
            ...
            if (GUILayout.Button("Start Bake"))
            {
                ...
                {
                    sVisualizationMode = PoiSDFBaker.DebugVisualization.Current;
    ...
    private void DebugPanel()
    {
        ...
        PoiSDFBaker.DebugVisualization newVisualization = (PoiSDFBaker.DebugVisualization)EditorGUILayout.EnumPopup("Visualization Mode", sVisualizationMode);

        //only draw the dialation slider for visualizations that need it
        float newVvisualizationDilation = sVvisualizationDilation;
        if (sVisualizationMode != PoiSDFBaker.DebugVisualization.Geometry && sVisualizationMode != PoiSDFBaker.DebugVisualization.None)
        {
            float minValue = sVisualizationMode == PoiSDFBaker.DebugVisualization.Unsigned ? 0.0f : -0.5f;
    ...
    private bool RebuildRendererList(GameObject newTargetObject, out List<PoiSDFBaker.TargetRendererInfo> newTargetRendererInfos)
    {
        PoiSDFBaker baker = (PoiSDFBaker)target;

        newTargetRendererInfos = new List<PoiSDFBaker.TargetRendererInfo>(0);
        ...
        for (int rendererIndex = 0; rendererIndex < newRenderers.Length; rendererIndex++)
        {
            ...
            if (currentRenderer != null && currentMesh != null)
            {
                    ...
                    && (currentMesh == baker.GetTargetMesh(rendererIndex)))
                {
                    ...
                    newTargetRendererInfos.Add(new PoiSDFBaker.TargetRendererInfo(currentRenderer, currentMesh, currentRendererToggle, currentSubmeshToggles));
                }
                else
                {
                    //add a new renderer info if these ones wern't already stored
                    newTargetRendererInfos.Add(new PoiSDFBaker.TargetRendererInfo(currentRenderer, currentMesh));
                }
            }
        }
